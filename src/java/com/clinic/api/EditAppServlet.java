...
public class EditAppServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String appNo = request.getParameter("appointment_number");
        ...
        try {
            ...
            if (con != null) {
                String query = "UPDATE appointments SET patient_name=?, contact_number=?, dentist=?, treatment=?, app_date=?, app_time=? WHERE appointment_number=?";
                PreparedStatement pst = con.prepareStatement(query);
                pst.setString(1, name);
                pst.setString(2, contact);
                pst.setString(3, dentist);
                pst.setString(4, treatment);
                pst.setString(5, date);
                pst.setString(6, time);
                pst.setString(7, appNo);
                pst.executeUpdate();
            }
